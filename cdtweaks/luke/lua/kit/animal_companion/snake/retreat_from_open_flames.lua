...
function %INNATE_SNAKE_RETREAT_FROM_OPEN_FLAMES%(op403CGameEffect, CGameEffect, CGameSprite)
	...
	if CGameEffect.m_effectId == 0xC and EEex_IsMaskSet(CGameEffect.m_dWFlags, dmgtype["FIRE"]) then -- Damage (FIRE)
		if spriteActiveStats.m_nResistFire < 100 then -- only apply if the target is not immune to fire
			...
			if not GT_Sprite_HasBounceEffects(CGameSprite, CGameEffect.m_spellLevel, CGameEffect.m_projectileType, CGameEffect.m_school, CGameEffect.m_secondaryType, CGameEffect.m_sourceRes:get(), {12, 23, 146, 24}, CGameEffect.m_flags) then
				if not GT_Sprite_HasImmunityEffects(CGameSprite, CGameEffect.m_spellLevel, CGameEffect.m_projectileType, CGameEffect.m_school, CGameEffect.m_secondaryType, CGameEffect.m_sourceRes:get(), {12, 23, 146, 24}, CGameEffect.m_flags, CGameEffect.m_savingThrow) then
