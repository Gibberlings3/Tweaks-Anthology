...
EEex_Sprite_AddQuickListsCheckedListener(function(sprite, resref, changeAmount)
	...
	if selectedWeapon["slot"] == 10 or selectedWeapon["slot"] == 34 or selectedWeapon["ability"].type ~= 1 then
		...
		-- store target id
		spriteAux["gt_NWN_SmiteGood_TargetID"] = curAction.m_acteeID.m_Instance
...
-- Forget about ``spriteAux["gt_NWN_SmiteGood_TargetID"]`` if the player manually interrupts the action --
...
EEex_Action_AddSpriteStartedActionListener(function(sprite, action)
	...
	if sprite:getLocalInt("gtNWNSmiteGood") > 0 then
		if not (action.m_actionID == 113 and action.m_string1.m_pchData:get() == "%BLACKGUARD_SMITE_GOOD%") then
			if spriteAux["gt_NWN_SmiteGood_TargetID"] ~= nil then
	...
	if sprite:getLocalInt("gtNWNSmiteGood") > 0 then
		if not (selectedWeapon["slot"] == 10 or selectedWeapon["slot"] == 34 or selectedWeapon["ability"].type ~= 1) then
			if sprite.m_nSequence == 0 and sprite.m_attackFrame == 6 then -- SetSequence(SEQ_ATTACK)
				if spriteAux["gt_NWN_SmiteGood_TargetID"] then
...
function %BLACKGUARD_SMITE_GOOD%(CGameEffect, CGameSprite)
	...
	-- op120
	local conditionalString = 'WeaponEffectiveVs(EEex_Target("gtSmiteGoodTarget"),MAINHAND)'
	-- alignment check
	local align = GT_Resource_SymbolToIDS["align"]
	local isGood = GT_Sprite_CheckIDS(CGameSprite, align["MASK_GOOD"], 8)
	--
	if GT_EvalConditional["parseConditionalString"](sourceSprite, CGameSprite, conditionalString, "gtSmiteGoodTarget") then
		if isGood then
