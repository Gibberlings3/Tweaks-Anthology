...
function GT_Sprite_HasBounceEffects(sprite, level, projectileType, school, sectype, resref, opcodes, flags)
	...
	local func = function(effect)
		if effect.m_effectId == 0xC5 and effect.m_dWFlags == projectileType and sectype ~= 4 then -- Physical mirror (197) -> CANNOT bounce effects of Secondary Type ``MagicAttack``
			toReturn = true
			return true
		elseif effect.m_effectId == 0xC6 and GT_LuaTool_ArrayContains(opcodes, effect.m_dWFlags) and sectype ~= 4 then -- Reflect specified effect (198) -> CANNOT bounce effects of Secondary Type ``MagicAttack``
			toReturn = true
			return true
